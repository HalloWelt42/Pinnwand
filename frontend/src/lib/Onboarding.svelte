<script lang="ts">
  ...
  const SCHRITTE: Schritt[] = [
    { titel: 'Willkommen bei Pinnwand', text: 'Eine lokale, modulare Plattform fuer Aufgaben, Zeiten und Planung. Diese kurze Einfuehrung zeigt die wichtigsten Bereiche. Du kannst sie jederzeit ueberspringen.' },
    { titel: 'Board und Spalten', text: 'Im Board legst du Aufgaben an und verschiebst sie per Drag-and-drop. Markiere im Spalten-Menue eine Spalte als Erledigt-Spalte - das nutzen Auswertung und Schnell-Erledigt.', ansicht: 'board' },
    { titel: 'Personen und Wochenstunden', text: 'Unter Planung legst du Personen mit ihrem Wochen-Soll je Wochentag an, traegst Urlaub ein (auch halbe Tage) und importierst Feiertage (Vorschau vor Uebernahme).', ansicht: 'planung' },
    { titel: 'Wiederkehrende Termine', text: 'Unter Serien richtest du wiederkehrende Termine/Aufgaben ein (z.B. eine woechentliche Telefonkonferenz). Sie werden automatisch als Karten mit Faelligkeit und Soll vorgebucht.', ansicht: 'serien' },
    ...
    { titel: 'KI ist optional', text: 'Vorlesen, Spracheingabe, semantische Suche und Transkriptionen sind optional. Ohne die Dienste laeuft alles weiter. Der Dienste-Status unten in der Seitenleiste zeigt, was erreichbar ist. Hilfe findest du jederzeit ueber das Fragezeichen.' },
  ]
...
<div class="overlay" role="presentation">
  <div class="karte" role="dialog" aria-label="Einfuehrung">
    <div class="punkte">
    ...
    <p>{aktuell.text}</p>
    <div class="fuss">
      <button class="text" onclick={onFertig}>Ueberspringen</button>
      <div class="rechts">
        {#if aktuell.ansicht}<button class="geist" onclick={gehe}>Dorthin</button>{/if}
        {#if i > 0}<button class="geist" onclick={() => (i -= 1)}>Zurueck</button>{/if}
